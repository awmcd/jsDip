...
support = function (order) {
    ...
    if (order.type === 'support_move' && !path(order_list[order.partner])) {
        return false;
    }
    ...
    _.each(order_list, function (other_order) {
        if (other_order.type === 'move' &&
            ...
            game[other_order.unit].unit.owner !== order.unit.owner &&
            path(other_order)) {
...
attack_strength = function (order) {
    ...
    // In all other cases...
    var strength = 1, destination_owner = order.destination.unit.owner;
        
    _.each(order_list, function (other_order) {
        
        if (other_order.type === 'support_move' &&
            other_order.destination === order.destination &&
            other_order.destination.unit.owner !== destination_owner &&
            other_order.result === true) {
...
move = function (order) {
    ...
    if (order.h2h) {
        if (attack_strength(order) > defend_strength(order_list[order.id])) {
            var outcome = true;
    ...
    if (!order.h2h) {
        if (attack_strength(order) > hold_strength(order_list[order.destination.id])) {
            var outcome = true;
...
function buildEquations (game, order_list) {
    return new equations(game, order_list);
}
