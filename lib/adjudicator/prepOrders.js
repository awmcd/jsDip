...
define(["underscore", "util/translateOrders",
        "util/trueAdjacency"], function (_, translateOrders, trueAdjacency) {
...
function prepOrders (game, state, order_list) {
    ...
    _.each(state.board, function (power) {
       ...
       _.each(power.units, function (unit) {
           ...
           if (!order_exists) {
               ...
               var hold_order = translateOrders(hold_order_string);
    ...
    // Update intra-order unit property to reflect game
    ...
    _.each(order_list, function (order) {
        ...
        var geo_check = trueAdjacency(order, game);
        
        // if not, is there a potentially-valid covoy path?
        if (!geo_check && order.unit.type === 'a' && game[order.unit.loc].type === 'coast' && order.type === 'move') {
            // ...
        ...
        if (!order.resolved && order.type === 'move') {
            if (game[order.destination].unit) {
                ...
                if (order_list[order.destination].type === 'move' &&
                    order_list[order.destination].destination === order.unit.loc) {
                    order.h2h = true;
                }
            }
        }
