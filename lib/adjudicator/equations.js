...
move = function (order) {
    ...
    if (order.h2h) {
        if (attack_strength(order) > defend_strength(order_list[order.unit.loc.id])) {
            console.log('!!!', order, attack_strength(order), defend_strength(order_list[order.unit.loc.id]))
            outcome = true;
        ...
        if (attack_strength(order) > hold_strength(order.destination)) {
            ...
            _.each(order_list, function (other_order) {
                if (other_order !== order &&
                    ...
                    if (prevent_strength(other_order) >= attack_strength(order)) {
                        console.log('::', order, prevent_strength(other_order), attack_strength(order))
                        outcome = false;
                    }
                }
            });
