...
function prepOrders (game, state, order_list) {
    ...
    _.each(order_list, function(order){
        ...
        if (game[order.destination].coasts) {
    ...
    _.each(order_list, function (order) {
        ...
        if (!geo_check && order.unit.type === 'a' && order.unit.loc.type === 'coast' && order.type === 'move') {
            // ...
        ...
        if (!order.resolved && order.type === 'move') {
            if (game[order.destination.id].unit) {
                ...
                if (order_list[order.destination.id].type === 'move' &&
                    order_list[order.destination.id].destination.id === order.unit.loc.id) {
                    order.h2h = true;
                }
            }
        }
